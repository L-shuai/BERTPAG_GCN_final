...
class Attention(nn.Module):
    def __init__(self, embed_dim, hidden_dim=None, out_dim=None, n_head=1, score_function='dot_product', dropout=0):
        ...
        self.proj = nn.Linear(n_head * hidden_dim, out_dim*2)
    ...
    def forward(self, k, q):#K,16*26*600   Q,16*600
        if len(q.shape) == 2:  # q_len missing #16*600
            q = torch.unsqueeze(q, dim=1)  #16*1*600
        if len(k.shape) == 2:  # k_len missing
            k = torch.unsqueeze(k, dim=1)
        mb_size = k.shape[0]  #16 ?
        k_len = k.shape[1]#26
        q_len = q.shape[1]#1
        # k: (?, k_len, embed_dim,)
        # q: (?, q_len, embed_dim,)
...
class Attention_Masked(nn.Module):
    ...
    def forward(self, k, q, k_step, q_step):
        ...
        q_len = q.shape[1]
        ipdb.set_trace()
        mask = torch.tensor([[0] * step + [-10000] * (k_len - step) for step in k_step.numpy()],
